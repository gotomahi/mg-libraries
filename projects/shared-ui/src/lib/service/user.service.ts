...
export class UserService {
  ...
  authenticate(token: any, username: string, password: string): Observable<string> {
    const authorization = 'Basic ' + btoa('address:password');
  ...
  isTokenValid(authentication: any): boolean {
    let valid = false;
    const remaingTime: number = new Date(authentication.exp).getTime() - new Date().getTime();
    valid = (remaingTime >= 10);
    return valid;
  }

  refreshAccessToken(token: any): Observable<any> {
    let authToken = this.dataShareService.token.getValue();
    const tokenPayload:any = atob(token.substring(8).split('.')[1]);
  ...
  getAccessToken(oldAccessToken: string): string {
    ...
    if(this.dataShareService.token.getValue()){
      accessToken = this.dataShareService.token.getValue().access_token;
    }
    return accessToken;
  }
